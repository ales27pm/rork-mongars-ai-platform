...
const SPM_PKG =
  "spm_pkg 'https://github.com/ml-explore/mlx-swift.git', :tag => 'v0.24.0'";
...
const SPM_MODULEMAP_CLEANUP = [
  ...
  "    end",
  "  end",
  "  patch_file = lambda do |file_path|",
  "    next unless file_path && File.exist?(file_path)",
  "    original = File.read(file_path)",
  "    updated = original",
  "    explicit_module_keys.each do |k|",
  '      updated = updated.gsub(/\\b#{Regexp.escape(k)}\\s*=\\s*YES;/, "#{k} = NO;")',
  "    end",
  "    updated = updated.gsub(/-fmodule-map-file(?:=|\\s+)(?:\\\"|')?[^\\s\\\"']*Cmlx\\.modulemap[^\\s\\\"']*(?:\\\"|')?/, '')",
  "    File.write(file_path, updated) if updated != original",
  "  end",
  "  installer.aggregate_targets.each do |aggregate_target|",
  "    Dir.glob(File.join(aggregate_target.support_files_dir, '*.xcconfig')).each do |xcconfig_path|",
  "      next unless File.exist?(xcconfig_path)",
  ...
  "    end",
  "  end",
  "  # Patch pbxproj(s) too: Xcode may persist experimental explicit-module settings and module-map flags there.",
  "  pods_pbxproj = File.join(installer.sandbox.root.to_s, 'Pods.xcodeproj', 'project.pbxproj')",
  "  patch_file.call(pods_pbxproj)",
  "  installer.aggregate_targets.each do |aggregate_target|",
  "    if aggregate_target.respond_to?(:user_project_path) && aggregate_target.user_project_path",
  "      user_pbxproj = File.join(aggregate_target.user_project_path.to_s, 'project.pbxproj')",
  "      patch_file.call(user_pbxproj)",
  "    end",
  "  end",
  "end",
].join("\n");
